...
parser.add_argument('--clean_dir', type=str, default='./data/nature_clean')
parser.add_argument('--noisy_dir', type=str, default='./data/nature_noise')
...
class DataLoader_tif(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #     noisy_im = noisy_im[xx:xx + self.patch, :, :]
        #     clean_im = clean_im[xx:xx + self.patch, :, :]
        # if W - self.patch > 0:
        #     yy = np.random.randint(0, W - self.patch)
        #     noisy_im = noisy_im[:, yy:yy + self.patch, :]
        #     clean_im = clean_im[:, yy:yy + self.patch, :]
...
#full dataset
full_dataset = DataLoader_tif(opt.noisy_dir, opt.clean_dir, patch=opt.patchsize)
...
#valid dataset
ValidLoader = DataLoader(dataset = val_dataset, num_workers=8, 
                            batch_size = 1,
                            shuffle=False, 
                            pin_memory=False,
                            drop_last = False)
...
for epoch in range(0, opt.n_epoch + 1):
    ...
    for iteration, (noisy, clean) in enumerate(TrainingLoader):
        ...
        print(
            ...
            .format(epoch, iteration, np.mean(loss1.item()), time.time() - st))
